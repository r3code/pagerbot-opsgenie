...
func (a *Api) Schedules() (Schedules, error) {
	...
	var opts opsgenie.ListSchedulesOptions
...
func updateScheduleListFromResponse(a *Api, res *opsgenie.ListSchedulesResponse, schdList *Schedules) error {
	...
	for _, bareSchedule := range res.Schedules {
		...
		res, err := a.client.GetSchedule(bareSchedule.ID, opsgenie.GetScheduleOptions{
			TimeZone: a.timezone,
			Since:    today,
			Until:    nextWeek,
		})
